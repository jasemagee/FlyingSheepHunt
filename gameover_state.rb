...
class GameoverState < StateBase
	...
	def draw
		@font_large.draw_rel("GAME OVER", 
			@window.width / 2, @window.height / 2, 0, #x, y, z
			0.5, 0.5, #rel_x, rel_y
			1.0, 1.0, COLOR)

		@font_large.draw_rel("SCORE #{@score}", 
			@window.width / 2, (@window.height / 2) + @font_large.height, 0, #x, y, z
			0.5, 0.5, #rel_x, rel_y
			1.0, 1.0, COLOR)

		@font_small.draw_rel("Press Esc", 
			@window.width / 2, (@window.height / 2) + (@font_large.height * 2), 0, #x, y, z
			0.5, 0.5, #rel_x, rel_y
			1.0, 1.0, COLOR)
	end
